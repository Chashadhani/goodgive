...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm p-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Edit Help Category</h1>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                <p class="text-sm text-gray-600">
                    ...
                    <strong>Help Requests:</strong> {{ $category->helpRequests()->count() }} request(s) using this category
                </p>
            </div>
            ...
            <div class="bg-gray-50 rounded-xl p-4">
                ...
                <div class="border-2 border-gray-200 rounded-2xl p-4 sm:p-6 text-center bg-white max-w-[200px]">
                    <div class="text-4xl mb-3" id="preview-icon">{{ $category->icon }}</div>
                    <h3 class="font-semibold text-gray-800" id="preview-name">{{ $category->name }}</h3>
                    <p class="text-xs text-gray-500 mt-1" id="preview-desc">{{ $category->description ?? 'Description here' }}</p>
                </div>
            </div>
