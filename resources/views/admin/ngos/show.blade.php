...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.ngos.index') }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-700">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6">
            <div class="flex items-start justify-between mb-6">
                <div class="flex items-center">
                    <div class="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center">
                        <span class="text-indigo-600 font-bold text-2xl">{{ strtoupper(substr($user->ngoProfile->organization_name ?? $user->name, 0, 2)) }}</span>
                    </div>
                    <div class="ml-4">
                        <h1 class="text-2xl font-bold text-gray-900">{{ $user->ngoProfile->organization_name ?? 'N/A' }}</h1>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-gray-900">{{ $user->ngoProfile->registration_number ?? 'N/A' }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Contact Person</h3>
                    <p class="text-gray-900">{{ $user->ngoProfile->contact_person ?? $user->name }}</p>
                </div>
                <div>
                    <h3 class="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Phone</h3>
                    <p class="text-gray-900">{{ $user->ngoProfile->phone ?? 'N/A' }}</p>
                </div>
                <div>
                ...
                <div class="col-span-2">
                    ...
                    <p class="text-gray-900">{{ $user->ngoProfile->address ?? 'N/A' }}</p>
                </div>
            </div>

            @if($user->ngoProfile?->verification_status === 'rejected' && $user->ngoProfile?->rejection_reason)
            ...
            @if($user->ngoProfile?->verified_at)
                <div class="mt-6 p-4 bg-green-50 rounded-lg border border-green-200">
                    <h3 class="text-sm font-medium text-green-800 mb-1">Verified On</h3>
                    <p class="text-green-700">{{ $user->ngoProfile->verified_at->format('M d, Y \a\t h:i A') }}</p>
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 mt-6">
            ...
            @if($user->ngoProfile?->documents)
                <div class="border border-gray-200 rounded-lg p-4">
                    <p class="text-gray-600">{{ $user->ngoProfile->documents }}</p>
                </div>
            @else
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-2xl shadow-sm p-6">
            ...
            <div class="space-y-3">
                @if($user->ngoProfile?->verification_status !== 'verified')
                    <form action="{{ route('admin.ngos.verify', $user) }}" method="POST">
                        @csrf
                        @method('PATCH')
                ...
                @if($user->ngoProfile?->verification_status !== 'pending')
                    <form action="{{ route('admin.ngos.pending', $user) }}" method="POST">
                        @csrf
                        @method('PATCH')
                ...
                @if($user->ngoProfile?->verification_status !== 'rejected')
            ...
            <div id="rejectSection" class="hidden mt-4">
                <form action="{{ route('admin.ngos.reject', $user) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="mb-3">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 mt-6">
            ...
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">Total Donations Received</span>
                ...
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">People Helped</span>
                    <span class="font-semibold text-gray-900">0</span>
                </div>
            </div>
        </div>
