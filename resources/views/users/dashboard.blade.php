...
<body class="font-sans antialiased bg-gray-50">
    ...
    <div class="min-h-screen py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Active Donations Coming Your Way -->
            @php
                $myRequestIds = Auth::user()->helpRequests()->pluck('id');
                $recipientAllocations = \App\Models\Allocation::where('allocatable_type', \App\Models\HelpRequest::class)
                    ->whereIn('allocatable_id', $myRequestIds)
            ...
            @if($recipientAllocations->count() > 0)
                <div class="bg-teal-50 border-2 border-teal-300 rounded-2xl p-6 mb-8">
                    <div class="flex items-start space-x-4">
                        <div class="w-12 h-12 bg-teal-200 rounded-full flex items-center justify-center flex-shrink-0">
                            <span class="text-2xl">🎁</span>
                        </div>
                        <div class="flex-1">
                            <h3 class="text-lg font-bold text-teal-800">Donations On The Way!</h3>
                            <p class="text-teal-700 mt-1">{{ $recipientAllocations->count() }} donation(s) have been allocated to your help requests.</p>
                            <div class="mt-4 space-y-3">
                                @foreach($recipientAllocations as $allocation)
                                    <div class="bg-white rounded-xl p-4 flex items-center justify-between border border-teal-200">
                                        <div>
                                        ...
                                        <a href="{{ route('recipient.requests.tracking', $allocation->allocatable) }}" 
