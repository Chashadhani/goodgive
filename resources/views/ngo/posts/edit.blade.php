    ...
    <div class="min-h-screen py-12">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form action="{{ route('ngo.posts.update', $ngoPost) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8"
                x-data="{ 
                    requestType: '{{ old('request_type', $ngoPost->request_type) }}',
                    items: {{ $ngoPost->isGoods() && $ngoPost->items->count() ? $ngoPost->items->map(fn($i) => ['item_name' => $i->item_name, 'quantity' => $i->quantity, 'notes' => $i->notes ?? ''])->toJson() : '[{\"item_name\":\"\",\"quantity\":1,\"notes\":\"\"}]' }},
